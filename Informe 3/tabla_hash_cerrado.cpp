////////  IMPLEMENTACION DE TABLA DE HASH CERRADA, CON LINEAR PROBING ////////
...
public: 
	...
	TablaHashCerrada(){ 
		...
		//A partir de la implementacion vista en clases, el vector de hash será de pares char, keydata
		//donde char es un caracter que indicara si la celda esta vacia, ocupada o borrada
		hash = new vector<pair<char, keydata>>(hash_primos[0]);//inicializamos la tabla con el tamaño primo mas pequeño (53)
		for(int i = 0; i< hash_primos[real_size]; i++){  //y seteamos el char de todas las celads en v
			(*hash)[i].first = 'v'; //inicializamos la hashTable en 'v'acio, para cada elemento
		}
	}
	...
	bool insert(keydata data){    
		//en este caso los elementos a insertar serán dados por la clave (string)
		float alpha_plus = (float)(size + 1) / hash_primos[real_size];   // Primero vemos si la insercion de este elemento sobrepasa el factor de carga
		
		// Si lo sobrepasa, resize
		if(alpha_plus > 0.75){
			resize();		
		}
		//Ahora calculamos su indice mediante la función hash
		...
		// Verificar si hay colision (Linear Probing)
		while((*hash)[index].first == 'o'){ //Si hay casillas ocupadas
			index = (index + 1) % hash_primos[real_size]; //vemos las siguientes (mod) tamaño del vector
		}
		
		// Cuando encuentre un espacio, lo inserta 
		(*hash)[index].first = 'o'; //seteamos su caracer a ocupado
		(*hash)[index].second = data; //y guardamos el keydata

		size++; //e incrementamos size en 1
	...
	bool buscar(string key){
		int index = hash_foo(key) % hash_primos[real_size]; //Calculamos el indice que resulta al aplicar la funcion hash en la key
		
		while((*hash)[index].first !='v'){  //Para los buckets no vacíos, 
			if((*hash)[index].first == 'b'){ //si son los que estan borrados, los saltamos
				index = (index + 1) % hash_primos[real_size];
				continue;
			}
			if(key.compare((*hash)[index].second.first) == 0){ //Si la key del elemento en la posicion index es igual a la key en cuestion
				cout<<"CLAVE ENCONTRADA EN TABLA, SU VALOR ES: ";  
				cout<<(*hash)[index].second.second[0]<<" "<<(*hash)[index].second.second[1]<<" ";
				cout<<(*hash)[index].second.second[2]<<" "<<(*hash)[index].second.second[3]<<" ";
				cout<<(*hash)[index].second.second[4]<<" "<<(*hash)[index].second.second[5]<<" ";
				cout<<(*hash)[index].second.second[6]<<endl;
				return true; //Imprimimos el valor de la key y retornamos true
			}

			index = (index + 1) % hash_primos[real_size]; //(incremento de 1 mod tamaño de la tabla en index, pues las inserciones se hicieron con linear probing)
	...
	void print(){
		for(int i = 0; i<(*hash).size();i++){
			...
			else if((*hash)[i].first == 'o'){
				cout<<"Fila "<<i<<" :";
				cout<<((*hash)[i].second).second[0]<<" " <<((*hash)[i].second).second[1]<<" ";
				cout<< ((*hash)[i].second).second[2]<<" "<<((*hash)[i].second).second[3]<<" ";
				cout<< ((*hash)[i].second).second[4]<<" " <<((*hash)[i].second).second[5]<<" ";
				cout<< ((*hash)[i].second).second[6]<<endl;
			}
			else{
				cout<<"Fila " <<i<< "con: " <<(*hash)[i].first<<endl;
			}
		}
	}
...
private:
	...
	vector<pair<char, keydata>> *hash; //el vector de pares caracter/keydata que representa a la tabla

	//FUNCIÓN RESIZE /////
	void resize(){
		//Primero creamos el vector mas grande
		vector<pair<char, keydata>> *aux = new vector<pair<char, keydata>>(hash_primos[real_size + 1]);
		//Ahora debemos inicializar en 'v' los elementos del vector auxiliar
		for(int i = 0; i< hash_primos[real_size + 1]; i++){
			(*aux)[i].first = 'v';
		}
		//AHora para los primeros hash_primos[real_size]

		for(int i = 0; i < hash_primos[real_size]; i++){
			if((*hash)[i].first == 'o'){ //Si estaban ocupados, rehasheamos con el nuevo real_size 
				int index = hash_foo((*hash)[i].second.first) % hash_primos[real_size + 1];

				while((*aux)[index].first == 'o'){ //Si esta ocupada la posicion index, hacemos linear probing
                	index = (index + 1) % hash_primos[real_size + 1];  
				}
				(*aux)[index].first = 'o';
				(*aux)[index].second = (*hash)[i].second;
			}
			//Las posiciones no ocupadas ya deberían estar marcadas como vacio, pues es lo que hicimos en el primer for
...
int main(){
	...
	getline(my_file, line);//Leemos la primera fila (de etiquetas)
	vector<string> linestring; //declaramos el vector de strings linestring
	TablaHashCerrada Tabla; //Declaramos la tabla hash
